/* Name: Phan Bảo Trân
 ID: ITDSIU21125
 Purpose: The Ship class is essential for managing the state and rendering of ships in a Battleship game. It tracks the position, orientation, length, and destruction state of the ship. The class includes methods for rendering the ship in both horizontal and vertical orientations, toggling its orientation, and updating its placement state. It also provides utility methods to retrieve the positions occupied by the ship and to check if the ship has been completely destroyed. This class forms a crucial part of the game's logic and user interface, ensuring that ships are correctly placed, tracked, and displayed on the game grid. 
*/
import java.awt.*;
